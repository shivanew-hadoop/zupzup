...
const DG_ENDPOINTING_MS = Number(process.env.DG_ENDPOINTING_MS || 80);
...
wss.on('connection', (client, req) => {
  ...
  const dg = deepgram.listen.live({
    ...
    vad_events: true,
    // Keep formatting light for faster interim captions. Final formatting can be added later.
    smart_format: false,
    punctuate: false,
  });
  ...
  dg.on(LiveTranscriptionEvents.Open, () => {
    dgReady = true;
    client.send(JSON.stringify({ type: 'status', text: `Deepgram connected (${DG_MODEL}). Listening with low latency...` }));
  ...
  dg.on(LiveTranscriptionEvents.Transcript, (data) => {
    ...
    if (!text) return;
    console.log('[Deepgram] Transcript:', text, data?.is_final ? '(final)' : '(interim)');
  ...
  client.on('message', (chunk) => {
    ...
    if (dgReady) {
      ...
      if (audioBuffer.length > 25) audioBuffer.shift();
    }
  });
